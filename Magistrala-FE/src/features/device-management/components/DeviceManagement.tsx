...
import { useAuth, AdminOnly, AuthorizedOnly } from '@/contexts/AuthContext';
...
export function DeviceManagement() {
  ...
  const loadDevices = async () => {
    try {
      ...
      // Admin can see all devices, user only their organization's devices
      const organizationFilter = isUser ? user?.organizationId : undefined;
  ...
  const handleUpdateDevice = async (deviceId: string, deviceData: Partial<Device>) => {
    try {
      ...
      <div className="flex justify-between items-center">
        ...
        <AdminOnly>
          <Button 
            ...
            className="bg-[#474dff] hover:bg-[#474dff]/90"
          >
    ...
    <Card className="hover:shadow-md transition-shadow">
      ...
      <CardContent>
        <div className="space-y-2 text-sm text-gray-600">
          ...
          <p><strong>Last Seen:</strong> {new Date(device.last_seen).toLocaleDateString()}</p>
        </div>
